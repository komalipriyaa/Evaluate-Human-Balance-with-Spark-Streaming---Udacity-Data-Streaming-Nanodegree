...
spark1 = SparkSession.builder.appName("customer-fallrisk").getOrCreate()
...
redisDF = spark1.readStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092").option("subscribe","redis-server").option("startingOffsets","earliest").load() 

# TO-DO: cast the value column in the streaming dataframe as a STRING 
kafkaRedisDF = redisDF.selectExpr("CAST(value AS string) value")
...
kafkaRedisDF.withColumn("value", from_json("value", redisSchema)).select(col('value.key'), col('value.existType'), col('value.Ch'),col('value.Incr'), col('value.zSetEntries')).createOrReplaceTempView("RedisSortedSet")
...
redisZSetDF = spark1.sql("select key, zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
#+--------------------+
#
redisZSetDecodedDF=redisZSetDF.withColumn("customer", unbase64(redisZSetDF.encodedCustomer).cast("string"))

# with this JSON format: {"customerName":"Sam Test","email":"sam.test@test.com","phone":"8015551212","birthDay":"2001-01-03"}
redisZSetDecodedDF.withColumn("customer", from_json("customer", customerSchema)).select(col('customer.*')).createOrReplaceTempView("CustomerRecords")
...
emailAndBirthDayStreamingDF = spark1.sql("select * from CustomerRecords where email is not null and birthDay is not null")
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
stediEventsRawDF = spark1.readStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092").option("subscribe", "stedi-events").option("startingOffsets", "earliest").load()   
...
stediEventsDF.withColumn("value", from_json("value", scoreSchema)).select(col('value.customer'), col('value.score'), col('value.riskDate')).createOrReplaceTempView("CustomerRisk")

# TO-DO: execute a sql statement against a temporary view, selecting the customer and the score from the temporary view, creating a dataframe called customerRiskStreamingDF

customerRiskStreamingDF = spark1.sql("select customer, score from CustomerRisk")


# TO-DO: join the streaming dataframes on the email address to get the risk score and the birth year in the same dataframe
customerRiskScoreDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr("""email=customer"""))
...
customerRiskScoreDF.selectExpr("to_json(struct(*)) as value").writeStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092").option("topic", "human-balance").option("failOnDataLoss", "false").option("checkpointLocation", "/tmp/kafkacheckpoint").start().awaitTermination()