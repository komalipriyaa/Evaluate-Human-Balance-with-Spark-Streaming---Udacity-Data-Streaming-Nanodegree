...
spark1 = SparkSession.builder.appName("redis").getOrCreate()
...
kafkaRedisDF.withColumn("value", from_json("value", redisMessageSchema)).select(col('value.key'), col('value.existType'), col('value.Ch'),col('value.Incr'), col('value.zSetEntries')).createOrReplaceTempView("RedisSortedSet")
